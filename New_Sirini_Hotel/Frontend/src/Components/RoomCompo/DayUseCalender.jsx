...
const DayUseCalender = ({
  ...
  const buildRange = () => {
    if (!selectedDate || !selectedCheckOut || timeSlot !== "fullday") return new Set();
    const range = new Set();
  ...
  const isConflict = (proposedStart, proposedEnd) => {
    return activeBookings.some((b) => {
      ...
      const bStart = b.timeSlot === "day" ? bIn + 12 * 3600000 : bIn + 16 * 3600000;
      const bEnd   = b.timeSlot === "day" ? bIn + 15 * 3600000 : bOut + 10 * 3600000;
  ...
  const getDayStatus = (day) => {
    ...
    if (timeSlot === "day") {
      const hasFullDayEdge = activeBookings.some(
        (b) => b.timeSlot === "fullday" && (b.checkInDate.split("T")[0] === dateStr || b.checkOutDate.split("T")[0] === dateStr)
      );
  ...
  const isNextDisabled = !selectedDate || (timeSlot === "fullday" && !selectedCheckOut);
  ...
  return (
    <div className="relative w-full max-w-[360px] bg-white/95 backdrop-blur-2xl rounded-[2rem] shadow-[0_32px_64px_-16px_rgba(0,0,0,0.1)] shadow-orange-500/5 p-5 border border-white/50 animate-in fade-in slide-in-from-bottom-4 duration-500 ease-out">
      {/* Close */}
      <button onClick={onClose} className="absolute top-5 right-5 text-gray-300 hover:text-gray-900 transition-colors">
        <X size={18} />
      </button>

      {/* Back */}
      <button onClick={onBack} className="group text-gray-400 text-[9px] font-black uppercase tracking-[0.2em] flex items-center gap-2 hover:text-orange-500 transition-colors mb-3">
      ...
      <div className="mb-3 relative">
        <div className="inline-flex items-center gap-2 bg-gradient-to-r from-orange-50 to-amber-50 border border-orange-100/50 rounded-xl px-3 py-1.5 mb-2 shadow-sm shadow-orange-500/5">
          ...
          <span className="text-orange-600 text-[9px] font-black uppercase tracking-[0.2em]">{pkg.label}</span>
        </div>
      ...
      <div className="flex gap-4 items-center bg-gray-50/50 border border-gray-100/80 p-1 px-4 rounded-2xl mb-3">
        <button onClick={() => setCurrentDate(new Date(year, month - 1, 1))} className="text-gray-400 hover:text-orange-500 hover:bg-white p-1 rounded-lg transition-all shadow-sm shadow-transparent hover:shadow-gray-200/50">
          <ChevronLeft size={16} />
        </button>
      ...
      <div className="grid grid-cols-7 gap-1 mb-3">
        {weekDays.map((d, i) => (
          <div key={`day-${i}`} className="text-center text-[7px] font-black text-gray-400 uppercase tracking-widest mb-0.5">{d}</div>
        ))}
        ...
        {Array.from({ length: daysInMonth }, (_, i) => i + 1).map((day) => {
          ...
          return (
            <button
              key={day}
              disabled={status === "past" || status === "blocked" || status === "span-blocked"}
              onClick={() => onDateSelect(dateStr)}
              title={
                status === "blocked" ? "Not available on this date" : 
                status === "span-blocked" ? "Cannot span across a blocked date" : undefined
              }
      ...
      <div className="flex justify-center gap-x-3 gap-y-1 mb-3 pt-2 border-t border-gray-50 flex-wrap">
        <div className="flex items-center gap-1">
          ...
          <span className="text-[7px] font-bold text-gray-400 uppercase tracking-widest">Available</span>
        </div>
        <div className="flex items-center gap-1">
          <div className="w-1.5 h-1.5 rounded-full bg-orange-500" />
          <span className="text-[7px] font-bold text-gray-400 uppercase tracking-widest">Selected</span>
        </div>
        {isFullDay && (
          <>
            <div className="flex items-center gap-1">
              <div className="w-1.5 h-1.5 rounded-full bg-orange-200 border border-orange-300" />
              <span className="text-[7px] font-bold text-gray-400 uppercase tracking-widest">Stay Range</span>
            </div>
            <div className="flex items-center gap-1">
              <div className="w-1.5 h-1.5 rounded-full bg-blue-400" />
              <span className="text-[7px] font-bold text-gray-400 uppercase tracking-widest">Day Booked</span>
            </div>
          </>
        )}
        {!isFullDay && (
          <div className="flex items-center gap-1">
            ...
            <span className="text-[7px] font-bold text-gray-400 uppercase tracking-widest">Check-in/out</span>
          </div>
        )}
        <div className="flex items-center gap-1">
          <div className="w-1.5 h-1.5 rounded-full bg-red-300" />
          <span className="text-[7px] font-bold text-gray-400 uppercase tracking-widest">Not Available</span>
        </div>
      </div>
      ...
      <button
        ...
        className="group relative w-full py-2.5 px-5 rounded-xl bg-gray-900 text-white font-black uppercase text-[9px] tracking-[0.25em] shadow-lg shadow-gray-900/20 hover:shadow-orange-500/20 disabled:opacity-30 disabled:grayscale disabled:cursor-not-allowed transition-all duration-500 overflow-hidden active:scale-[0.98] flex items-center justify-center gap-2 h-10"
      >
        ...
        <span className="relative z-10 flex items-center gap-2">
          {!selectedDate
            ? (isFullDay ? "Confirm Dates" : "Pick a Date")
            : isFullDay
            ? `${selectedDate}  →  ${selectedCheckOut || "Check-out"}`
            : selectedDate}
