...
import { 
  User, CheckCircle2, XCircle, History, Archive, 
  Flag, Search, Trash2, Clock, Check, X, Phone, Calendar
} from "lucide-react";
...
function RoomBookedDetails({ refreshKey, onActionCompleted }) {
  ...
  const handleBookingAction = async (id, actionType) => {
    ...
    try {
      let endpoint = "";
      if (actionType === "confirm") endpoint = `http://localhost:5000/api/rooms/confirmbooking/${id}`;
      else if (actionType === "cancel") endpoint = `http://localhost:5000/api/rooms/cancelbooking/${id}`;
      else if (actionType === "complete") endpoint = `http://localhost:5000/api/rooms/completebooking/${id}`;
  ...
  if (loading) return <div className="text-center py-20 text-gray-400 italic tracking-widest">LOADING RECORDS...</div>;
      ...
      <div className="flex flex-col xl:flex-row justify-between items-center gap-6">
        ...
        <div className="relative w-full xl:w-80">
          ...
          <input 
            ...
            className="w-full pl-12 pr-4 py-3.5 bg-white border border-gray-200 rounded-2xl text-sm outline-none focus:ring-4 focus:ring-black/5 transition-all" 
      ...
      {filteredList.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 gap-6">
          {filteredList.map((req) => (
            <div 
              key={req._id} 
              className="bg-white rounded-[2rem] p-6 shadow-sm border border-gray-100 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 flex flex-col group"
            >
              {/* Header: User Info & Status */}
              <div className="flex justify-between items-start mb-5">
                <div className="flex items-center gap-3">
                  <div className="w-12 h-12 rounded-2xl bg-gray-50 flex items-center justify-center text-gray-400 border border-gray-100 group-hover:bg-black group-hover:text-white transition-colors duration-500">
                    <User size={24} />
                  </div>
                  <div>
                    <h4 className="font-black text-gray-900 text-lg leading-tight uppercase tracking-tighter">{req.name}</h4>
              ...
              <div className="bg-gray-50 rounded-2xl p-4 space-y-3 mb-6">
                <div className="flex justify-between items-center">
                  <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Room Number</span>
                  <span className="font-mono font-black text-black text-lg">{req.roomNumber}</span>
                </div>
                ...
                {activeTab === "pending" && (
                  <>
                    <button 
                      ...
                      className="flex-1 bg-green-500 text-white py-3 rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-black transition-all shadow-lg shadow-green-500/20 active:scale-95"
                    >
                      Approve
                    </button>
                    <button 
                ...
                {activeTab === "approved" && (
                  <>
                    <button 
                      ...
                      className="flex-1 bg-black text-white py-3 rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-[#D4AF37] transition-all active:scale-95"
                    >
                      Mark Completed
                    </button>
                    <button 
...
const TabButton = ({ active, onClick, icon, label, count, color }) => (
  <button 
    onClick={onClick} 
    className={`flex items-center gap-2 px-6 py-3 rounded-xl text-[11px] uppercase tracking-widest transition-all whitespace-nowrap
    font-sans antialiased
    ${active 
        ? `bg-white ${color} shadow-md ring-1 ring-black/5 font-medium` 
...
const StatusBadge = ({ status, tab }) => {
  ...
  return (
    <span className={`inline-flex items-center gap-1 px-3 py-1.5 rounded-lg text-[9px] font-black uppercase border-2 shadow-sm ${styles[tab] || "bg-gray-50 text-gray-500"}`}>
      {status}
    </span>
  );
};
