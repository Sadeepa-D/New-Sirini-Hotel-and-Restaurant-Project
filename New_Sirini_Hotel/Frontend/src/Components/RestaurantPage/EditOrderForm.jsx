...
export default function EditOrderForm({ item = {}, editingOrder, onClose }) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setSubmitted(true);
            toast.success(`Order ${editingOrder ? 'updated' : 'placed'} successfully!`);
            setTimeout(() => {
                onClose();
            }, 2000);
        } catch (error) {
                        ...
                        <div className="mt-2 p-4 bg-amber-50 border border-amber-200 rounded-xl">
                            <div className="flex justify-between items-center">
                                <span className="text-sm text-amber-800">
                                    Rs. {(item.price || (editingOrder ? editingOrder.Price / editingOrder.quantity : 0)).toLocaleString()} × {form.quantity}
                                </span>
                                <span className="text-xl font-bold text-amber-700">
                                    Rs. {((item.price || (editingOrder ? editingOrder.Price / editingOrder.quantity : 0)) * form.quantity).toLocaleString()}
                                </span>
                            </div>
                        </div>
